...
public class UKRDCIndexManagerUpdateRecordSystemTest {
	...
	public static void setup()  throws MpiException {
		...
		MasterRecordDAO.deleteByNationalId("UPDT2000R1",MasterRecord.UKRDC_TYPE);
		clear( "UPDT20001", "UPDT2");
		MasterRecordDAO.deleteByNationalId("UPDT3000R1",MasterRecord.UKRDC_TYPE);
		MasterRecordDAO.deleteByNationalId("UPDT3000R2",MasterRecord.UKRDC_TYPE);
		MasterRecordDAO.deleteByNationalId("UPDT3000R3",MasterRecord.UKRDC_TYPE);
		clear( "UPDT30001", "UPDT3");
		clear( "UPDT3A001", "UPDT3A");
		MasterRecordDAO.deleteByNationalId("UPDT4000R1",MasterRecord.UKRDC_TYPE);
		MasterRecordDAO.deleteByNationalId("UPDT4000N1",MasterRecord.NHS_TYPE);
		MasterRecordDAO.deleteByNationalId("UPDT4000C1",MasterRecord.CHI_TYPE);
		MasterRecordDAO.deleteByNationalId("UPDT4000H1",MasterRecord.HSI_TYPE);
	...
	public void test2UpdateAddingUKRDCId() throws MpiException {
		...
		p1.setPrimaryIdType(MasterRecord.UKRDC_TYPE).setPrimaryId(idBase+"R1");
		im.createOrUpdate(p1);
		// VERIFY
		Person person = PersonDAO.findByLocalId(p1.getLocalIdType(), p1.getLocalId(), p1.getOriginator());
		assert(person!=null);
		MasterRecord master = MasterRecordDAO.findByNationalId(idBase+"R1",MasterRecord.UKRDC_TYPE);
		assert(master!=null);
		List<LinkRecord> links = LinkRecordDAO.findByPerson(person.getId());
		assert(links.size()==1);
		...
		person = PersonDAO.findByLocalId(p1.getLocalIdType(), p1.getLocalId(), p1.getOriginator());
		assert(person!=null);
		master = MasterRecordDAO.findByNationalId(idBase+"R1",MasterRecord.UKRDC_TYPE);
		assert(master!=null);
		links = LinkRecordDAO.findByPerson(person.getId());
		assert(links.size()==1);
		assert(links.get(0).getMasterId()==master.getId());
	...
	public void test3UpdateChangingUKRDCId() throws MpiException {
		...
		p1.setId(0); // reset the id - incoming record won't have this.
		p1.setPrimaryIdType(MasterRecord.UKRDC_TYPE).setPrimaryId(idBase+"R2");
		im.createOrUpdate(p1);
		// VERIFY
		Person person = PersonDAO.findByLocalId(p1.getLocalIdType(), p1.getLocalId(), p1.getOriginator());
		assert(person!=null);
		MasterRecord master = MasterRecordDAO.findByNationalId(idBase+"R1",MasterRecord.UKRDC_TYPE);
		assert(master==null);
		MasterRecord master2 = MasterRecordDAO.findByNationalId(idBase+"R2",MasterRecord.UKRDC_TYPE);
		assert(master2!=null);
		List<LinkRecord> links = LinkRecordDAO.findByPerson(person.getId());
		assert(links.size()==1);
		...
		p2.setPrimaryIdType(MasterRecord.UKRDC_TYPE).setPrimaryId(idBase+"R2");
		...
		p1.setId(0); // reset the id - incoming record won't have this.
		p1.setPrimaryIdType(MasterRecord.UKRDC_TYPE).setPrimaryId(idBase+"R3");
		im.createOrUpdate(p1);
		// VERIFY
		person = PersonDAO.findByLocalId(p1.getLocalIdType(), p1.getLocalId(), p1.getOriginator());
		assert(person!=null);
		master = MasterRecordDAO.findByNationalId(idBase+"R2",MasterRecord.UKRDC_TYPE);
		assert(master!=null);
		master2 = MasterRecordDAO.findByNationalId(idBase+"R3",MasterRecord.UKRDC_TYPE);
		assert(master2!=null);
		links = LinkRecordDAO.findByPerson(person.getId());
		assert(links.size()==1);
		assert(links.get(0).getMasterId()==master2.getId());
	...
	public void test4AddingOtherNationalId() throws MpiException {
		...
		p1.setId(0); // reset the id - incoming record won't have this.
		p1.addNationalId(new NationalIdentity(MasterRecord.NHS_TYPE,idBase+"N1"));
		p1.addNationalId(new NationalIdentity(MasterRecord.CHI_TYPE,idBase+"C1"));
		im.createOrUpdate(p1);
		// VERIFY
		Person person = PersonDAO.findByLocalId(p1.getLocalIdType(), p1.getLocalId(), p1.getOriginator());
		assert(person!=null);
		MasterRecord masterUkrdc = MasterRecordDAO.findByNationalId(idBase+"R1",MasterRecord.UKRDC_TYPE);
		assert(masterUkrdc!=null);
		MasterRecord masterNhs = MasterRecordDAO.findByNationalId(idBase+"N1",MasterRecord.NHS_TYPE);
		assert(masterNhs!=null);
		MasterRecord masterChi = MasterRecordDAO.findByNationalId(idBase+"C1",MasterRecord.CHI_TYPE);
		...
		p1.addNationalId(new NationalIdentity(MasterRecord.NHS_TYPE,idBase+"N1"));
		p1.addNationalId(new NationalIdentity(MasterRecord.HSI_TYPE,idBase+"H1"));
		im.createOrUpdate(p1);
		// VERIFY
		person = PersonDAO.findByLocalId(p1.getLocalIdType(), p1.getLocalId(), p1.getOriginator());
		assert(person!=null);
		masterUkrdc = MasterRecordDAO.findByNationalId(idBase+"R1",MasterRecord.UKRDC_TYPE);
		assert(masterUkrdc!=null);
		masterNhs = MasterRecordDAO.findByNationalId(idBase+"N1",MasterRecord.NHS_TYPE);
		assert(masterNhs!=null);
		masterChi = MasterRecordDAO.findByNationalId(idBase+"C1",MasterRecord.CHI_TYPE);
		assert(masterChi==null);
		MasterRecord masterHsi = MasterRecordDAO.findByNationalId(idBase+"H1",MasterRecord.HSI_TYPE);
		assert(masterHsi!=null);

		links = LinkRecordDAO.findByPerson(person.getId());
		assert(links.size()==3);
		assert(links.get(0).getMasterId()==masterUkrdc.getId());
		assert(links.get(1).getMasterId()==masterNhs.getId());
		assert(links.get(2).getMasterId()==masterHsi.getId());
		items = WorkItemDAO.findByPerson(person.getId());
		assert(items.size()==0);
		...
		masterChi = MasterRecordDAO.findByNationalId(idBase+"C1",MasterRecord.CHI_TYPE);
		assert(masterChi==null);
		masterHsi = MasterRecordDAO.findByNationalId(idBase+"H1",MasterRecord.HSI_TYPE);
		assert(masterHsi!=null);
		assert(masterHsi.getSurname().equals("DARBY"));
