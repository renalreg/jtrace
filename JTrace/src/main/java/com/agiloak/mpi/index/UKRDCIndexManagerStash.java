...
public class UKRDCIndexManagerStash {
	
	private final static Logger logger = LoggerFactory.getLogger(UKRDCIndexManagerStash.class);
	...
	public void createOrUpdate(Person person) throws MpiException {
		...
		if (storedPerson != null) {
			...
			if (storedPerson.getPrimaryId()==null) {
				...
				if  (person.getPrimaryId()==null || person.getPrimaryId().length()==0) {
				...
				if (nationalIdMatch(storedPerson.getPrimaryId(), person.getPrimaryId(),
									storedPerson.getPrimaryIdType(), person.getPrimaryIdType())) {
		...
		if ((person.getPrimaryIdType() != null) && (person.getPrimaryId() != null)) {
			...
			if (master != null) {
				...
				if (linkedPersons.size()==0) {
					...
					MasterRecordDAO.deleteByNationalId(person.getPrimaryId(), person.getPrimaryIdType());
				} else {
				...
				logger.warn("deleteMasterRecords unable to find a master for:"+person.getPrimaryIdType()+":"+person.getPrimaryId());
			}
	...
	protected boolean verifyMatch(Person person, MasterRecord master) {
		...
		if (person.getPrimaryIdType() == null || person.getPrimaryIdType().length()==0) {
			return nomatch;
		}
		if (!person.getPrimaryIdType().equals(master.getNationalIdType())) {
			return nomatch;
		}
		...
		if (!person.getPrimaryId().equals(master.getNationalId())) {
			return nomatch;
		}
